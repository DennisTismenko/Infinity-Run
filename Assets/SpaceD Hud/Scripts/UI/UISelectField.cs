...
[ExecuteInEditMode]
[AddComponentMenu("SpaceD UI/Select Field")]
public class UISelectField : UIWidgetContainer
{
	...
	public bool isOpen { get { return this.listRoot != null; } }
	...
	void OnSelect(bool isSelected)
	{
		...
		{
			this.arrowElement.spriteName = (isSelected) ? this.arrowOpenedSprite : this.arrowClosedSprite;
	...
	private void CreateList()
	{
		...
		for (int i = 0, imax = this.items.Count; i < imax; ++i)
		{
			...
			if (i != (imax - 1) && !string.IsNullOrEmpty(this.listSeparatorSprite))
			{
				...
				int sepUsedSpace = Mathf.RoundToInt(separator.CalculateBounds().size.y) + (this.listSepMarginTop + this.listSepMarginBottom);
